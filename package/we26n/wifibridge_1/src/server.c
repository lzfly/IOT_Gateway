...
int get_Deviceid_config(char *jyconfig)
{
    ...
    else
    {
        ...
        const char *value_w = NULL;
        const char *value_h = NULL;
...
int get_SleepTime_config(char *ennconfig)
{
    ...
    {
        const char *value_sp = NULL;
        const char *value_sw = NULL;
        const char *value_sh = NULL;
...
int get_By_multiplication_factor( int connectfd, char * buff_heat_take )
{
    ...
    crcvalue_b = modbus_crc16( (uint8_t *)buff_by, len_b - 2 );
    printf("crcvalue_b = %x\n", crcvalue_b);
...
void * enn_meter_thread( void * arg )
{
    ...
    while(1)
    {
        ...
        crcvalue = modbus_crc16( (uint8_t *)buf_power, len_p - 2 );
        printf("crcvalue = %x\n", crcvalue);
        ...
        crcvalue = modbus_crc16( (uint8_t *)buf_water, len_w - 2 );
        printf("crcvalue = %x\n", crcvalue);
        ...
        crcvalue = modbus_crc16( (uint8_t *)buf_heat, len_h - 2 );
        printf("crcvalue = %x\n", crcvalue);
...
int main(int argc,char *argv[])
{
    ...
    while(1)
    {
        ...
        ret = pthread_create( &th, NULL, enn_meter_thread, (void *)(intptr_t)connectfd );
        if( ret != 0 )
        {
